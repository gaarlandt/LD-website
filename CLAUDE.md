...
**Convention — app icons are hand-authored (not generated):** the PWA app icons (`public/icons/icon-{192,512}.png` + full-bleed `maskable-{192,512}.png`) and the iOS `app/apple-icon.png` are hand-authored "Silver" raster assets, committed directly. `scripts/generate-icons.mjs` (run by `npm run assets`) now generates **only** the legacy `favicon.ico` — it deliberately does **not** touch the app icons (it can't reproduce the silver design from the flat `icon-black.svg`), so `npm run assets` is safe to run and won't clobber them. To refresh the set, re-export from the design tool and drop the PNGs into `public/icons/` + `app/apple-icon.png`. The apple-touch-icon must stay **opaque** (iOS renders transparency as black — the squircle is flattened onto a silver field). Manifest icons carry both `purpose: "any"` (squircle) and `purpose: "maskable"` (full-bleed). The browser-tab favicon stays the wordmark `app/icon.svg`.
