...
- **CTA tracking**: `components/analytics/cta-tracker.tsx` mounts a delegated document click listener that fires `cta_clicked` (via `trackEvent`) for links to `app.letsdog.nl`, `keuzehulp.letsdog.nl`, `agenda.letsdog.nl`, the staging checkout host (`staging-app.letsdog.nl` → `link_destination:"checkout"`), and same-site pricing links (`/prijzen` + `#prijzen` → `"pricing"`). Params `link_url`, `link_text`, `link_location` (navbar/body), `link_destination` — **the last two are registered GA4 custom dimensions; do not rename.** Pricing additionally fires GA4-ecommerce `view_item_list` (`components/sections/pricing-view-tracker.tsx`) + `begin_checkout` (`components/sections/plan-cta.tsx`, with `billing_period` monthly/yearly + `items`). App-side `sign_up`/`purchase` live on `app.letsdog.nl` (owned by Jur). See the GA4 setup doc (`Tech/GA4 LD/`).
