...
- **Error sink**: Sentry (EU ingest, project `website`), since 2026-08-15 (T-44, loop decision D-6). No SDK anywhere — the browser (`lib/error-sink.ts`) POSTs an envelope built by the dependency-free `lib/error-report.ts`, tagged `runtime: browser`. (Until 2026-09-14 the contact Pages Function was a second runtime in the same project, tagged `pages-function`; it was retired with T-83, so older Sentry history still carries that tag.) **`@sentry/browser` was built and then removed on a measurement**: its barrel is the only entry point and Turbopack does not tree-shake it behind a dynamic import, so it added a 438 KB / 144 KB-gz chunk containing rrweb — the session-replay engine the cookie declaration §6 promises we do not run. Hand-rolling costs **+3.8 KB raw / +2.2 KB gz** total. Reported: the handover-cookie contract breaches (`ld_attribution` rule 5, both cookies' rule 6), the forms' round trip to the platform failing (`contact.platform_unreachable` / `_refused` / `_captcha_refused` — what only the browser can witness; the platform reports its own server-side failures), and a consent chain that never started. **Not** reported: unhandled browser errors (rejected in D-6). **Ungated** — it must speak for a visitor who refused everything — and the counterpart of that is a **hard limit: no consent categories in the payload**, enforced by an allowlist of measurement keys in `lib/error-report.ts` rather than by call-site discipline. Key `NEXT_PUBLIC_SENTRY_DSN` (Cloudflare dashboard only); **unset is a supported state and a total no-op**
...
- **Mobile header (homepage-only white state):** on `/` only, the fixed header shows a white hamburger + white logo (with a legibility drop-shadow) until scrolled past 16px, then flips to the existing dark ink + `beige/95` bar — the homepage's mobile hero is a full-bleed photo that dark icons can't reliably read against. Every other route keeps the dark icon/logo unconditionally (their top section is plain beige). Top-right on mobile sits **exactly one pill** (`order-4`), and which one depends on the route: the peach **Start gratis proef** CTA (→ `/prijzen`) everywhere, except on `/prijzen` itself where it becomes a **Login** pill (→ `mijn.letsdog.nl`) — there the CTA would point at the page you are already on. **The peach fill is opaque on purpose.** Until 2026-08-26 this pill was a translucent glass fill and this file claimed "its own translucent fill carries contrast regardless of what's behind it"; that was measured false — it borrowed its contrast from the photo underneath and scored 1,36 to 3,13:1 across seven routes (T-72). Ink on peach is a constant 9,6:1 regardless of background. The Login variant on `/prijzen` keeps a light translucent fill, which is safe **only** because that route never opens on a photo (see the white-state gate above). Gated via `usePathname()` in `components/layout/navbar.tsx`.

## Navigation Order
```
Rassenkeuze hulp | Puppycursus | Prijzen | Over ons | Partners | Contact
```
Defined in `components/layout/navbar.tsx` (desktop + mobile) and `components/layout/footer.tsx`. **FAQ left the top nav on 2026-08-03** to free that slot for Partners; `/veelgestelde-vragen/` is now reachable from the footer (last entry, alongside the footer-only Homepage link) and the 404 quick-links only — the two nav arrays are separate consts and stay in sync by convention, not by a shared import. Desktop navbar also includes an outlined **Inloggen** button (→ `mijn.letsdog.nl`) and a peach **Start gratis proef** CTA (→ `/prijzen`), the latter hidden on `/prijzen` itself. **Mobile** reorders the same bar instead of reusing the desktop CTAs: hamburger pinned far left (`order-1`), logo nudged right next to it (`order-2`), then a flexible spacer, then the single pill pinned far right (`order-4`) — see the white-state note above for which pill that is on which route. The mobile dropdown menu's only button is **Login**, and it renders only when the bar is showing the CTA; on `/prijzen` the bar already carries Login, so the menu carries none. (Until 2026-08-26 this was the other way round — a Login pill in the bar and a **Start vandaag** CTA in the menu. The swap put the selling action in front of the menu instead of behind it.)
...
- **Env vars** (set in Cloudflare Pages → Settings → Variables and Secrets, scoped to Production AND Preview):
  ...
  - `NODE_VERSION=20`
  - **Retired 2026-09-14 (T-83), safe to delete from the dashboard:** `POSTMARK_SERVER_TOKEN`, `CONTACT_TO`, `CONTACT_FROM`, `CREATORS_TO`, `TURNSTILE_SECRET_KEY`. They powered the contact Pages Function, which is gone; the forms post to the platform, which holds the Turnstile secret for this site's widget as `WEBSITE_TURNSTILE_SECRET_KEY`.
  - `NEXT_PUBLIC_TURNSTILE_SITE_KEY` (public Site Key — set as a **plaintext** variable; inlined into the client bundle at build) — the Cloudflare Turnstile widget on both forms, one **Managed** widget (hostnames `letsdog.nl` + `www.letsdog.nl` + `website-letsdog.pages.dev`) whose **secret lives on the platform** since 2026-09-14. Set in **Production** scope; leave Preview unset so branch previews render Cloudflare's always-pass **test** widget — whose token the platform refuses, so a preview form cannot deliver (by design).
  ...
  - `NEXT_PUBLIC_SENTRY_DSN` — the error sink (T-44 / D-6). **Plaintext**, scoped to Production AND Preview. Pages inlines it into the client bundle at build time for `lib/error-sink.ts` (until 2026-09-14 it also reached the contact Pages Function through `env`). A DSN is public by design (it ships in the bundle), so this is not a secret being mishandled, but it lives **only** in the dashboard. Unset or malformed ⇒ the sink is a silent no-op that breaks nothing; and as with every `NEXT_PUBLIC_*` here, **setting it changes nothing until a build runs.**
...
- **Multi-fire**: `lib/analytics.ts` `trackEvent(name, params)` fires the SAME event to GA4 (gtag) AND PostHog, each guarded independently (a blocked sink never suppresses the other). The **Meta Pixel is a third sink on the same chokepoint but receives only a mapped subset** — the events that correspond to a Meta *standard* event (`add_to_cart`→`AddToCart`, both form successes→`Lead`, `view_item_list`→`ViewContent`), translated in `lib/meta-events.ts`. `cta_clicked` is deliberately not sent. **Event ownership is split across two hosts (D-101, 2026-08-11)** now that both `letsdog.nl` and the platform carry a pixel into the same dataset: this site owns `PageView`/`ViewContent`/`AddToCart`/`Lead`, the platform owns `InitiateCheckout` (only it can see a real checkout arrival, for both ways in) and `Purchase` (server-side over the Conversions API, never in a browser). The pricing click therefore maps to `AddToCart`, not `InitiateCheckout` — an event both hosts fire is an event counted twice. **The same rule reached Google on 2026-09-14 (platform D-473):** the click's GA4 name is `add_to_cart`, because the platform sends `begin_checkout` on checkout arrival and the site's old `begin_checkout` made Google count two per visitor. The name lives in `components/sections/plan-cta-event.ts` and must move together with its `MAPPINGS` key in `lib/meta-events.ts` — an unknown name maps to `null` there without a sound. Check that table in `docs/analytics-events.md` before adding a Meta mapping. `identifyLead(email)` is the single PostHog `identify` (contact- and creator-form success) — **it identifies on the anonymous `$device_id`, never on the email address** (T-46, 2026-08-17, reversing the old contract rule: one shared PostHog project since 2026-08-12 made an email-as-`distinct_id` a personal datum in a shared space and in a `.letsdog.nl` cookie, and the cross-host join runs on the shared `$device_id` anyway). The address rides along only as a person property, and only behind an explicit statistics yes. No `alias()`. **Add events by calling `trackEvent` — don't call `gtag`/`posthog` directly.** Full event reference (every event, trigger, properties + PostHog super-properties): [`docs/analytics-events.md`](docs/analytics-events.md).
- **CTA tracking**: `components/analytics/cta-tracker.tsx` mounts a delegated document click listener that fires `cta_clicked` (via `trackEvent`) for links to `mijn.letsdog.nl` (and the retiring `app.letsdog.nl`, kept listed while that host resolves), `keuzehulp.letsdog.nl`, `agenda.letsdog.nl`, the production checkout path (`<app host>/checkout*` → `link_destination:"checkout"`, split by path from the general `app` host — the `?plan=` choice rides in the query, so the path split still applies), and same-site pricing links (`/prijzen` + `#prijzen` → `"pricing"`). Params `link_url`, `link_text`, `link_location` (navbar/body), `link_destination` — **the last two are registered GA4 custom dimensions; do not rename.** Pricing additionally fires GA4-ecommerce `view_item_list` (`components/sections/pricing-view-tracker.tsx`) + `add_to_cart` (`components/sections/plan-cta.tsx` via `plan-cta-event.ts`, with `billing_period` monthly/yearly + `items`; `begin_checkout` until 2026-09-14). App-side `sign_up`/`purchase` live on the platform (`mijn.letsdog.nl`, owned by Jur). See the GA4 setup doc (`Tech/GA4 LD/`).
...
- **Known ceiling (brand decision):** white on brand-green `#75876D` maxes at **3.86:1** — enough for large text (3:1), never for body (4.5:1), and unreachable without darkening the green (`#697B61` would give 4.56:1) or switching to dark text. Jur's call; re-confirmed 2026-08-21 that the green stays. **Never put a Tailwind opacity modifier on `--ld-on-green` text** — `/70`/`/75`/`/85` drop it to 2.74–3.27:1, which is drift *below* the brand's own accepted floor rather than a design decision, and it made the platform's accessibility statement (which cites 3.86:1 as a deliberate choice) untrue for this site. Removed in T-66; the hierarchy those modifiers implied is carried by size and weight alone.
- **Contrast maths gotcha — `html { font-size: 85% }`** (`app/globals.css`, back to 100% only at ≥1440px). Every `rem`-based size renders 15% smaller than its Tailwind name suggests below that width: `lg:text-2xl` is **20.4px**, not 24px, so the WCAG large-text exemption (≥24px) does **not** apply between 1024px and 1439px where a token-level reading says it does. Measure the rendered `px`, not the class name.
...
## Important Notes
- Static export: no Next.js server features (no API routes, no SSR), and **since 2026-09-14 no server code at all** (T-83, executing platform decisions D-355 / D-444). The contact form and the `/partners` creator application POST straight from the browser to the platform's `submit-website-contact` (`lib/website-contact.ts`), which checks the Origin, the honeypot, the fields and the Turnstile token, stores the message in its CMS (/admin/inzendingen) and sends the internal notification and the confirmation through Loops. The contract — URL, fields, answers, limits — is the platform's: hub `contracts/websiteformulier-ingang.md`; **report a field or set change to the platform first**. It posts directly rather than through a proxy because the platform verifies Turnstile and rate-limits against the visitor's IP. Allowed Origins are `letsdog.nl` + `www.letsdog.nl` only, so the form does not deliver on a branch preview or on `website-letsdog.pages.dev`. Metadata routes (`app/robots.ts`, `app/sitemap.ts`, `app/manifest.ts`) need `export const dynamic = "force-static"`.
