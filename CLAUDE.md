...
- **Analytics**: GA4 (`G-0FCGXJHMMY`, fires immediately) + PostHog (EU project 143695, browser-only) + Meta Pixel (`1789754812188603`, `<head>`, ungated) — all fired via `lib/analytics.ts` `trackEvent`; Meta receives only the mapped standard-event subset (`lib/meta-events.ts`). Cookiebot banner (display-only, does not gate tracking). Details: `docs/analytics-events.md`
...
│   └── analytics/          # Cookiebot, GA4, CTATracker, PostHogProvider, MetaPixel + MetaPixelPageView
...
- **Env vars** (set in Cloudflare Pages → Settings → Variables and Secrets, scoped to Production AND Preview):
  ...
  - `NEXT_PUBLIC_META_PIXEL_ID=1789754812188603` (Meta Pixel — public by design, it ships in page source; set as a **plaintext** variable, NOT a secret, so the build can inline it. Unset or malformed ⇒ `components/analytics/meta-pixel.tsx` renders nothing and the pixel silently does not exist)
...
- **Multi-fire**: `lib/analytics.ts` `trackEvent(name, params)` fires the SAME event to GA4 (gtag) AND PostHog, each guarded independently (a blocked sink never suppresses the other). The **Meta Pixel is a third sink on the same chokepoint but receives only a mapped subset** — the events that correspond to a Meta *standard* event (`begin_checkout`→`InitiateCheckout`, both form successes→`Lead`, `view_item_list`→`ViewContent`), translated in `lib/meta-events.ts`. `cta_clicked` is deliberately not sent. `Purchase` cannot fire here at all: payment happens on another domain. `identifyLead(email)` is the single PostHog `identify` (contact-form success) — lowercased email is the cross-product join key; no `alias()`. **Add events by calling `trackEvent` — don't call `gtag`/`posthog` directly.** Full event reference (every event, trigger, properties + PostHog super-properties): [`docs/analytics-events.md`](docs/analytics-events.md).
...
- **Cookiebot banner is display-only; GA4 + PostHog + the Meta Pixel fire pre-consent — a PENDING legal re-confirm, not a closed decision (#2).** The product owner accepted firing without consent gating (see the honest "consent theater" note + one-line revert in `components/analytics/ga4.tsx`, and the same note in `components/analytics/meta-pixel.tsx`), but firing non-essential analytics before consent implicates **ePrivacy Directive Art. 5(3)** (prior consent for non-essential tracking) and **GDPR Art. 6** (lawful basis). The Meta Pixel (added 2026-08-07, ungated by explicit owner decision with risk accepted) widens this from analytics to **advertising**, and `content/cookieverklaring.md` §3.3/§5 still promise advertentiepixels are placed only after consent while §4 names Meta Pixel — so code and published policy currently disagree. It needs sign-off from someone with **legal** authority — logged + dated — before the production domains flip; that gate is a pre-flight checklist item in `docs/CUTOVER.md`.
