    ...
    <footer className="bg-[#162A0E] text-white/70 -mt-10 rounded-t-[2.5rem] relative">
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-14 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-[1.6fr_1fr_1fr] gap-10 lg:gap-12 mb-12">
          ...
          {/* Navigatie */}
          <div>
            <p className="text-sm font-semibold text-white/90 mb-5 uppercase tracking-wider">
              Navigatie
            </p>
            <ul className="space-y-3" role="list">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="inline-block py-0.5 text-sm text-white/60 hover:text-white transition-colors duration-200"
                  >
                    {link.label}
                  </Link>
                </li>
        ...
        {/* Bottom bar */}
        <div className="border-t border-white/10 pt-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="flex items-center justify-center w-12 h-12 rounded-2xl border border-white/15 bg-white/[0.04] text-white/60 hover:bg-white/10 hover:text-white transition-colors duration-200"
            >
