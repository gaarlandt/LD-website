...
**Last session ended**: 2026-06-30. **Pricing copy reframed from "geld-terug-garantie" → "betaal pas na 7 dagen" + €0,10 iDEAL-mandate explained + checkout IDs swapped** (`feat/prijzen-billing-copy-and-links`) — client-only static, no server surface. The Early Member billing reality (only ~€0,10 is charged up front to set up a doorlopende iDEAL/Mollie machtiging; the full €59 is incasso'd only **after 7 days**, so cancelling within 7 days = no charge, nothing to refund) had been mis-described sitewide as a "7 dagen geld-terug-garantie". Reworded across `app/prijzen/page.tsx` (the "niet bij jullie past" + already-live "geld terug" FAQ answers) and `app/veelgestelde-vragen/faq-data.ts` ("Wat kost het abonnement?" + "Kan ik opzeggen?" reworded; **the "Wat als ik niet tevreden ben?" FAQ removed** per owner), plus **both `pricing-data.ts` footer badges** (Early → "Pas na 7 dagen betalen · eerste jaar €59", Flexibel → "Maandelijks opzegbaar · geen restitutie"). New FAQ **"Waarom wordt er eerst €0,10 afgeschreven?"** added to both the FAQ page and `/prijzen`. **Checkout product IDs swapped**: maand/Flexibel `2126`→`2234`, jaar/Early-Member `2127`→`2233` in `components/sections/pricing-data.ts` (`ctaHref` add-to-cart + coupled `productId`), with `docs/CUTOVER.md` + `docs/analytics-events.md` updated in lockstep. Build green; preview-verified (toggle swaps the CTA `2233`↔`2234`, both badges render, €0,10 FAQ present, 0 console errors). **Owner must confirm WooCommerce products `2233`/`2234` are live in the checkout before/at merge** — external, not verifiable from this repo. Full decision inventory in `pricing-copy-inventory.html` (working doc, root). Billing model saved to session memory.

**Earlier — 2026-06-25 (later).** **Full-site review remediation Phases B–G COMPLETE → prod.** All 17 remaining findings after Phase A are merged to `main` and live (plan `docs/plans/2026-06-23-001-fix-website-review-remediation-plan.md`; review `docs/reviews/2026-06-23-website-review/`), each its own PR + `release/*` tag (G = docs-direct-to-main). **B (PR #49):** `lib/content.ts` throws a slug-named, path-named build error instead of a bare ENOENT (U5); the contact-modal post-close 250ms reset timer is `useRef`-held + cancelled on reopen/unmount so a fast close→reopen can't wipe the Turnstile token (U6a), and a non-OK `/api/contact` maps server field codes (name/email/message) to field UI vs the generic banner (U6b). **C (PR #50):** removed unused `framer-motion` + dead `Reveal`/CSS keyframes (**lockfile regenerated on Node 20/npm 10** — the `6e2f8fd` trap) (U7); `NVGH Logo.jpeg`→`nvgh-logo.jpeg` + `OptimizedImage`/AVIF (U8); accordion `ref as any` dropped (U9); National2 heading preload via a stable `/fonts/` URL + React 19 `preload()` (Turbopack hashes relative `@font-face` urls, defeating a static preload) (U10). **D (PR #51):** CSP `/*` gains `object-src 'none'; base-uri 'self'; form-action 'self'` (U11). **E (PR #52):** raster Organization JSON-LD logo (U12); sitemap `lastmod`→build date + maskable-icon decision documented (U13); api-catalog advertises `POST /api/contact`, `llms.txt` gains ip-overdracht + €39,50 consult + the 6 puppycursus phases + lowercase brand (U14); footer order matches the navbar (U15). **F (PR #53):** pure-helper Vitest coverage — `buildEmbedUrl`→`lib/embed-url.ts`, `parseFrontMatter` extracted from `content.ts`, `parsePrice` exported, 3 test files + a `@/` Vitest alias; **49 tests pass** (U16). **G (direct to main `fb30d44`):** CLAUDE.md navbar CTA fixed (Inloggen→app, Start vandaag→/prijzen); pre-consent analytics recorded as a **pending legal re-confirm** (ePrivacy 5(3)/GDPR 6) with a pre-flight legal-sign-off gate added to `docs/CUTOVER.md` (U17). **`/ce-compound light`** captured 3 reusable learnings in `docs/solutions/`: the close→reopen timer race (cross-project candidate), server-error-code→field-UI mapping, and the stable-font-URL + `preload()` recipe. **Open follow-ups:** (1) **DONE — AbortController on the contact `fetch`** (PR #54, `release/contact-fetch-abort-*`): a 10s timeout + cancel-on-close on the `/api/contact` submit, so a stalled upstream no longer hangs the button and closing mid-submit can't leave stale state — the pre-existing reliability gap the Phase B review surfaced. (2) Cutover gates still open in `docs/CUTOVER.md` (owner, non-code): WAF rate-limit rule, Turnstile prod keys, and the **legal consent sign-off** (#2 / R4). Each phase was verified locally (build / `npm ci` / preview) before merge; owner reviewed + merged B–E and authorized auto-merge of D/F.

**Earlier the same day — Phase A.** **Contact-endpoint hardening — review remediation Phase A MERGED → prod** (PR #48, merge `efd6884`, tag `release/contact-hardening-20260625-123110`). First phase of the 2026-06-23 full-site code+security review remediation (plan `docs/plans/2026-06-23-001-fix-website-review-remediation-plan.md`; review at `docs/reviews/2026-06-23-website-review/`). Hardened the one server endpoint `functions/api/contact.ts` against the **P1 #1** branded-email amplification vector + supporting reliability/observability findings, **with the repo's first automated tests co-landed**. **U1 (#1):** dropped the replayed "Jouw bericht: {message}" echo from the customer confirmation (recipient is user-supplied, so echoing user content from our verified Postmark sender was the phishing primitive) — the **support** email keeps the full message; **capped the retained `Hoi {name}` greeting at 20 chars** (review residual R-B, owner-approved) so it can't carry a lure; **CR/LF-collapse `name`** at the source (field-injection guard for the unescaped support text/Subject). **U2 (#5/#6):** `AbortSignal.timeout` on the Turnstile (5s) + Postmark (10s) fetches + structured `console.error("[contact] …")` on every failure path (normal bot 400s stay unlogged). **U3 (#8):** replaced the exact-match enforced-hosts `Set` with `isPreviewOrLocalHost()` — fail-closed for unknown hosts; only localhost + branch `*.pages.dev` get the always-pass test secret; apex/www/prod-alias `website-letsdog.pages.dev` require a real secret (500 if unset). **U4 (#11):** bootstrapped **Vitest** (33 tests) + `tsconfig.test.json` so `next build`'s `**/*.ts` typecheck stays green. Reviewed via `/ce-code-review` (7 reviewers, Opus on security/adversarial/correctness — zero P0/P1 left); safe fixes folded in. **Two build traps caught only on the Cloudflare preview** (preview-first earned its keep): the first build FAILED because the npm-11/Node-24 lockfile omitted optional `@emnapi/*` deps that Cloudflare's Node-20/npm-10 `npm ci` requires → reconciled on Node 20 (new learning: `docs/solutions/developer-experience/vitest-bootstrap-next-static-export-cloudflare-pages.md`). Owner verified the live send (confirmation renders with no echo + capped greeting). **Cross-project learning captured:** the hub Postmark contract now carries the anti branded-email-amplification rule (`ld-project-cross-knowledge/contracts/postmark-transactional-email.md`, owner WS). **Cutover gates added to `docs/CUTOVER.md`:** WAF rate-limit rule on `POST /api/contact` (per-IP; per-recipient is the deferred real bound) + the R-A residual (prod hash/branch `*.pages.dev` aliases classify as preview — mooted by setting the prod Turnstile secret). **Plan Phases B–G are separate sessions** (U5 content guard · U6 modal race + field errors · U7 framer-motion removal · U8–U10 assets/font · U11 CSP · U12–U15 SEO/metadata, with a raster `.png` already added for U12 · U16 helper tests · U17 docs/consent record, direct-to-main).

**Earlier — 2026-06-22.** **Legal docs synced to legal-dept "versie 2" + pricing product-ID swap** (`feat/legal-v2-sync-pricing-ids`) — client-only static, no server surface. The 5 footer legal pages (`privacybeleid`, `cookieverklaring`, `algemene-voorwaarden`, `ai-gebruiksvoorwaarden`, `retour`) were **recreated verbatim from the legal department's versie-2 source docs** (Google Drive "Leidraad juridische documenten" folder) — rule: *"V2 leads, what's not in V2 is gone"* (so AV dropped its old App-Store / 18+ / indemnity / forum clauses; retour dropped the Flexibel/Early-Member commercial specifics) — with `mail@letsdog.nl`→`support@letsdog.nl` in the bodies and brand kept lowercase "Let's dog". `retour` **renamed** *annuleringsbeleid* → *herroepingsbeleid* (route stays `/retour`, no redirect). **Two new footer legal pages** per the Leidraad's footer rule: `/modelformulier-herroeping` (EU model withdrawal form) + `/veiligheidsdisclaimer` (dog-behaviour safety disclaimer) — wired into the footer (now **7 Beleid links**), `app/sitemap.ts`, and `public/llms.txt`. **Pricing product IDs swapped**: maand/Flexibel `2109`→`2126`, jaar/Early-Member `2107`→`2127` in `components/sections/pricing-data.ts` (`ctaHref` `add-to-cart` + the coupled `productId`), with `docs/CUTOVER.md` + `docs/analytics-events.md` updated in lockstep. **Left on `mail@letsdog.nl` by owner decision** (the site's *general* contact, distinct from `support@`): `content/ip-overdrachtsverklaring.md`, the contact page, and the Organization JSON-LD (`lib/structured-data.ts`). Build green; preview-verified (both new pages, the cookie GFM retention table, 7 footer links, sitemap, 4 refreshed pages — 0 console errors). Content is a faithful transcription — **legal/owner should still eyeball the rendered pages**.

**Earlier — 2026-06-17.** **Forward `/rassenkeuze/` query params into the BreedSelector iframe** (`feat/rassenkeuze-iframe-query-forward`) — client-only static, no server surface. The `/rassenkeuze/` "Doe de test" iframe block was extracted into a small `"use client"` child (`app/rassenkeuze/rassenkeuze-embed.tsx`); `page.tsx` stays a Server Component (keeps `metadata`). The child reads `window.location.search` in a `useEffect` (**not** `useSearchParams()` — that forces a Suspense boundary under `output: "export"`), forwards the whole incoming query into the `keuzehulp.letsdog.nl` iframe `src`, strips any incoming `source`, and forces `source=website`. No params → `https://keuzehulp.letsdog.nl/?source=website` (fresh quiz, behaviorally unchanged). The "Open in nieuw tabblad" fallback link forwards the same URL. Hydration-safe: a same-height placeholder renders until mount (server HTML + first client render both show it), then the iframe swaps in — also avoids a fresh-quiz→results double-load on the deep-link path. This powers BreedSelector's new results-email links that deep-link visitors back to their results; **BS** points those links here (test `website-letsdog.pages.dev/rassenkeuze/`, prod URL at cutover) as a one-line config switch. Note: letting localhost/preview hosts *render* the embed is a keuzehulp-side `frame-ancestors` change (BS owns it) — not needed here, the website side is verified at the DOM level. Build green; verified on local preview across deep-link / no-params / source-dedup cases (iframe `src` correct, all attributes preserved, 0 React console errors). Merged straight to prod (pages.dev) — owner waived preview-channel testing since the real BS deep-link test happens in prod once BS ships. Net-new `window.location.search`-under-static-export pattern — `/ce-compound` candidate.

**Also 2026-06-17** — **Pricing checkout flipped staging → production** (`feat/pricing-production-checkout-links`) — client-only static, no server surface. The two `/prijzen` + homepage tier CTAs now point at the live WooCommerce shop: **Flexibel/monthly → `app.letsdog.nl/checkout/?add-to-cart=2109`**, **Early Member/yearly → `…?add-to-cart=2107`** (the coupled `productId` fields in `components/sections/pricing-data.ts` were updated in lockstep so `begin_checkout` `item_id` follows). Production checkout shares the `app.letsdog.nl` host (not a separate host like staging was), so `components/analytics/cta-tracker.tsx` is now **path-aware**: `app.letsdog.nl/checkout/*` → `link_destination:"checkout"`, all other `app.letsdog.nl` links stay `"app"`; the old `staging-app.letsdog.nl` host entry was removed. Ticked the CUTOVER.md "Swap the pricing checkout" item. Verified on preview: both hrefs correct, 0 staging links left, `cta_clicked`→`"checkout"` + `begin_checkout`→`item_id:2109` fire on a pricing click, navbar "Inloggen" still →`"app"`. **Owner follow-up (GA4 dashboard, not code):** add `app.letsdog.nl` to GA4 cross-domain (CUTOVER.md next item) so a www→app→checkout journey is one session.

**Earlier — 2026-06-11.** **Site-wide copy tweaks + brand restyle** (`feat/site-copy-tweaks`) — client-only static, no server surface. **Brand "Let's Dog" → "Let's dog" (lowercase d)** across all website output: copy, page `<title>`s, OG/Twitter, alt/aria, manifest, and JSON-LD (`SITE_NAME` in `lib/seo.ts` cascades to Organization/WebSite/Product + `og:siteName`). Capital-D deliberately kept in 5 internal code comments + 2 CSS header comments (not user-facing); the `brand-guide-letsdog` skill was updated to match so generated copy stays lowercase. **Em-dashes stripped from visible body copy** site-wide (→ comma); kept in `<title>`/meta descriptions, alt/aria, the manifest name, and the one on-page plan-label separator ("Early Member — Jaarlijks") — per owner choice. **Homepage**: hero eyebrow → "Online puppycursus"; Probleem blokken reworded; "Wat je krijgt" → gezin / video's / gedragstherapeuten / oorzaken; teaser eyebrow "De puppycursus" → **"De puppyagenda"**, heading "Van geboorte tot pubertijd", **week-labels removed** from all 4 cards, **"Meer dan 130 lessen"** (was 150), block copy reworded; Trust **"80+ videolessen"** (was 50+); certificering subtitles → NVGH / Raad-van-Beheer membership wording; cert logo **+30%** (`w-full max-w-[416px] md:max-w-[499px]`, homepage + over-ons). **Over ons**: hero + mijn-verhaal → "zonder dwang, **op basis van vertrouwen**"; methode trimmed **4→3 cards** (Welzijn block removed → "Drie principes", grid `md:grid-cols-3`), reworded; certs → membership wording. Build green, homepage + over-ons preview-verified (0 console errors). No formal plan doc (ad-hoc owner bullet list).

**Earlier — 2026-06-04.** **"Jaar + Consult" pricing tier removed** (`feat/remove-jaar-consult-tier`) — security model: client-only static, no server surface. The third tier (€79/€119, incl. the 1× online consult) is gone from the shared `<Pricing>` component (`components/sections/pricing.tsx`); the two remaining tiers (**Flexibel** €12,99/mnd, **Early Member** €59/€99) now render in a **centered 2-up grid** (`lg:grid-cols-2 max-w-3xl mx-auto`, verified centered on both sides of the 1440px root-font breakpoint + mobile single-column). Swept every coupled mention of the bundle out of **live copy**: prijzen H1 + meta + geld-terug FAQ, the FAQ-page "Wat kost het abonnement?" answer (feeds FAQPage JSON-LD), the Product JSON-LD description (`lib/structured-data.ts`), `public/llms.txt`, and the **return-policy** heading (`content/retour.md` — site isn't live yet & nothing was sold, so safe to drop the legal reference). Built HTML confirmed **0** occurrences of "Jaar + Consult" across all pages. The standalone **paid consult service** (`app.letsdog.nl/consult/`, €39,50 — contact + over-ons) is a different product and is untouched. **Retires** the old open item to wire the "Kies Jaar + Consult" CTA.

**Earlier — 2026-06-02.** **Homepage restructure MERGED — PR #27** (merge `bb6f036`, tag `release/homepage-restructure-20260602-192325`). Replaced the stale **"In drie stappen aan de slag"** block (its "gratis aanmelden" step contradicted the €12,99/mnd pricing) with a new **static** **"De puppyagenda"** 4-phase teaser (`components/sections/puppy-agenda-teaser.tsx` — Vóór de komst 15 / De eerste week thuis 8 / Wennen & socialiseren 10 / Ontdekken & groeien 20 lessen; **"Meer dan 150 lessen"** = full program, not the sum of the 4 shown; lesson-type chips checklist·video·audio·**gezondheid**, gezondheid = documented off-palette coral `#C2554B`, Phosphor `Heartbeat`). Homepage reordered to **Hero → Probleem → Wat je krijgt → Puppyagenda → Trust → Prijzen → Final CTA → slim rassenkeuze strip** (`components/sections/rassenkeuze-strip.tsx`, soft-blue P.S.). **"Wat je krijgt"** trimmed to outcomes + the on-the-go benefit folded in with a phone icon; its redundant "Puppyagenda voor elke week" callout **and** "Start vandaag" CTA removed. **App-store badges moved into the footer** ("Download de app"; Google Play live, App Store "binnenkort"; **no PWA copy** — PWA parked as a future to-do/FAQ). Final CTA → `#prijzen` (inline pricing). `how-it-works.tsx` + `breed-selector.tsx` **deleted**. Trust keeps its own "50+ videolessen" stat so 150+ isn't shown twice. All new sections are **server components** (static-export-safe). Plan: [`docs/plans/2026-06-02-001-feat-homepage-restructure-plan.md`](docs/plans/2026-06-02-001-feat-homepage-restructure-plan.md). New learning: [`docs/solutions/developer-experience/fresh-worktree-needs-npm-install.md`](docs/solutions/developer-experience/fresh-worktree-needs-npm-install.md) (a fresh worktree has no `node_modules`; the parent checkout's install was stale → build failed on `@radix-ui/react-label` until `npm install` in the worktree). The signed-off visual mockup `homepage-final-flow.html` was a repo-root scratch artifact (not committed).
...
**Session log:** 2026-06-03 — homepage copy hooks + PA tooltip image (`feat/homepage-copy-hooks`, plan [`docs/plans/2026-06-03-001-feat-homepage-copy-hooks-plan.md`](docs/plans/2026-06-03-001-feat-homepage-copy-hooks-plan.md)) — security model: client-only static, no auth-adjacent data. Copy sharpened to the rust/vertrouwen + week-voor-week positioning: hero H1 ("Meer rust en vertrouwen, samen met je pup.") + subtext, hope-intro, final-CTA kop ("Hoe eerder je begint, hoe rustiger het wordt."), footer tagline, PA closing-CTA (dropped the now-defunct "gratis account" promise). Pricing: consult €39 → €39,50, totale waarde €138 → €138,50, removed the 4,8-sterren social-proof regel (+ unused `Star` import). Trust: bare "2" trainers-stat → `Medal` icon sized to the numbers' cell height. **Beyond the literal plan, for consistency + to satisfy the plan's own 0-hits grep check:** also swept the old "Puppytraining die werkt" / "dagelijks plan" / "Jouw pup begrijpen. Samen groeien." positioning out of the sitewide default metadata (`app/layout.tsx`) and the PWA manifest (`app/manifest.ts`). PA tooltip: replaced `public/images/pa-tooltip.png` (the STAP 6/12 pop-up that had a black border) with a clean recreation — transparent PNG, baked rounded corners + soft shadow, 317×290 — regenerated the AVIF/WebP variants, updated `SHOTS.tooltip` dims, and dropped the redundant overflow/border/box-shadow wrapper in `progress.tsx`. Verified on the dev preview (DOM + screenshots): copy live, medal balanced, tooltip renders clean & borderless.
