...
**Session log:** 2026-06-03 — PWA "Silver" icon set wired in (`feat/pwa-icons-silver`) — security model: client-only static, no auth-adjacent data. Adopted the regenerated silver squircle + balloon-dog icons: replaced `public/icons/icon-192/512.png`, added full-bleed `maskable-192/512.png`, and `app/manifest.ts` now declares real `maskable` entries (was reusing the transparent 512 for the maskable slot — the Lighthouse maskable gap). iOS `app/apple-icon.png` = the 180 squircle **flattened onto opaque silver** — iOS renders transparency as black, and the raw squircle has transparent corners, so apple-touch must be opaque. **`scripts/generate-icons.mjs` no longer generates the app icons** (it can't reproduce the silver raster from the flat `icon-black.svg`); it now emits only the legacy `favicon.ico`, so `npm run assets` won't clobber the hand-authored icons (verified: checksums identical after a run). Tab favicon stays the wordmark `app/icon.svg`. Build + dev-server verified (manifest lists 4 icons, all serve 200). New convention added to CLAUDE.md.
