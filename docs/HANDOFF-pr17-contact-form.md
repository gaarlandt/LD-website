...
> **2026-05-31 diagnosis of the "Er ging iets mis" error (CORRECTED).** The function **is deployed and running** on the branch preview — the working URL is the **28-char-truncated** alias `https://feature-contact-page-redesig.website-letsdog.pages.dev` (Cloudflare caps branch aliases at 28 chars, dropping the trailing `n`; the full-length name 404s as "Deployment Not Found"). On the correct URL, `POST /api/contact` returns **`{"ok":false,"error":"server_not_configured"}` (HTTP 500)** — that is the function's own guard for **a missing `POSTMARK_SERVER_TOKEN`**. So it's not a code or Postmark-rejection bug: **the token isn't reaching the function.** Two causes, likely both: (1) **scope** — the branch preview reads the **Preview** env scope; a token set on **Production only** is invisible to it; (2) **timing** — Cloudflare applies env changes only to **new deployments**, so a token added after the last build needs a **fresh build** to take effect.
> **Fix:** ensure `POSTMARK_SERVER_TOKEN` is set on the **Preview** scope (CF → `website-letsdog` → Settings → Variables and Secrets), then **trigger a new deployment** (Deployments → Retry/redeploy, or push any commit to the branch). Re-test: `curl -X POST https://feature-contact-page-redesig.website-letsdog.pages.dev/api/contact -H 'Content-Type: application/json' -d '{"name":"t","email":"t@e.com","message":"hi"}'` → expect `{"ok":true}`. **Next gate after that:** if it then returns **502 `send_failed`**, the `CONTACT_FROM` sender (`noreply@letsdog.nl`) isn't verified in Postmark — verify it. (Merging to `main` also works as the "fresh deployment" — production then serves the function with the Production-scoped token.)
