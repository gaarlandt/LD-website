...
| **Edit pricing** | edit the **`tiers`** array in `components/sections/pricing-data.ts` — the Product/Offer JSON-LD reads it; derived figures (per-year, savings %, per-month) recompute from `priceValue` in `pricing-plans.tsx` | Google Rich Results test |
...
| **Build an interactive overlay** (popover / `<dialog>` / disclosure toggle) | prefer HTML **invoker commands** — `command`/`commandfor` on a `<button>` + the Popover API / native `<dialog>` — over a hand-rolled `useState`+`onClick`+ARIA toggle ([spec `recommended`](https://specification.website/spec/foundations/invoker-commands/), Baseline end-2025; keep a JS fallback, React 19 may need a TS cast). Don't rip out a working Radix modal for it — the win is bespoke popovers/tooltips (e.g. `components/layout/app-store-coming-soon.tsx`). Cross-project contract: `…/LD - project cross knowledge/contracts/invoker-commands.md` | keyboard (Enter/Space) opens/closes with no script; DevTools shows the `command` event firing on the target |
