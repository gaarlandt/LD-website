...
| Rate-limit rule (`/api/contact`) | ✅ Free-plan rule live; burst blocks from the 6th request (5 / 10s, Block). **Retired 2026-09-14 (T-83):** nothing serves `/api/contact` any more, so this zone rule and its alert guard a dead path — whether to delete them is Jur's call |
| Contact form + Turnstile | ✅ delivers; tokenless POST → `400` (fail-closed on the apex). **Retired 2026-09-14 (T-83):** the forms post to the platform's `submit-website-contact`, which holds the Turnstile secret as `WEBSITE_TURNSTILE_SECRET_KEY` and rate-limits per address and per IP itself |
...
- **Cookie consent + colour contrast — deferred.** ~~Pre-consent analytics stays — legal sign-off obtained 2026-08-07, posture unchanged (loop D-1), and it now covers the Meta Pixel too~~ **SUPERSEDED 2026-08-08 by loop D-93: consent is now enforced for Google and Meta — see "Consent posture" below**; brand-green small-text contrast (~3.86:1) is a brand decision. Both consciously left.
...
| 7 | Legal sign-off on pre-consent analytics — covers GA4, PostHog **and** the Meta Pixel | ✅ done — loop **D-1**, then *superseded 2026-08-08 by loop **D-93*** (gating is now on) |
...
> **2026-09-14 (T-83): the four contact-form items below describe the retired Pages Function.** They stay ticked as the
> history they are. Since that date the contact and creator forms post from the browser to the platform's
> `submit-website-contact` (`lib/website-contact.ts`; hub contract `contracts/websiteformulier-ingang.md`): no Postmark
> token, no `TURNSTILE_SECRET_KEY` and no `/api/contact` on this site. Checking the form now means a real submission on
> the apex, measured in the platform's `website_messages` and `email_log`.
...
- [~] **Verify the PostHog cross-subdomain cookie.** *(Our half is done: the `ph_…` cookie is set on the apex and `cross_subdomain_cookie: true` ships in the provider. Still open: confirm on the real apex that the `ph_…` distinct-id cookie has `Domain=.letsdog.nl` — it scopes to `pages.dev` on preview, so this only resolves post-cutover.)*
  **The rest of this item was rewritten on 2026-08-12; the original asked for something that is no longer the plan.** It read: "Coordinate `app.letsdog.nl` to init the same PostHog project (143695) with `cross_subdomain_cookie:true` and call `posthog.identify('wp:<id>', { email:<lowercased> })` on login." Three things in that sentence expired at the platform cutover. `app.letsdog.nl` is the retiring WordPress environment, not the counterpart — that is `mijn.letsdog.nl`. `wp:<id>` was a WordPress user id and no such id exists on the platform, which identifies on the Supabase uuid. And the direction reversed: rather than the other host joining 143695, **this site moved to the platform's project** (Jur, 2026-08-12), so 143695 is left to BreedSelector.
  **Two reasons this item can no longer be closed by the website alone, and neither is fixed by the flag we ship.** The platform runs the React Native SDK, which does not read posthog-js's cookie at all — so a shared project still does not hand the visitor across. And the two sides identify differently (lowercased email here, Supabase uuid there), which stays two persons even inside one project. Both are tracked as **T-542 in the LDplatform loop**; nothing here closes them.
...
**Consent posture — reversed 2026-08-08 (supersedes the 2026-08-07 sign-off).** Loop decision **D-93** replaces the ungated posture that D-1 had signed off: consent is now enforced for Google and Meta. Google tags start from a Consent Mode v2 `denied` default, and the Meta Pixel does not load until marketing consent is granted. PostHog is explicitly out of that scope — it runs on legitimate interest and only stops on an explicit refusal. Details in `docs/analytics-events.md` → "Consent posture".
