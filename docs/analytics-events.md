...
`PageView` fires twice per journey-start by design of the two mechanisms combined: once from the base code on hard load, then once per App Router **soft** navigation via [`meta-pixel-pageview.tsx`](../components/analytics/meta-pixel-pageview.tsx). The first client render is skipped so a landing isn't double-counted. Without that component the whole site would report a single PageView per session and page-based retargeting audiences would stay empty.
